node {
  ...
  switch (env.BRANCH_NAME) {
    ...
    case "master":
        ...
        sh("sed -i.bak 's#gcr.io/${project}/${appName}:0.0.2#${imageTag}#' ./k8s/deployment.yaml")
        sh("kubectl --namespace=prod apply -f k8s/service.yaml")
        sh("kubectl --namespace=prod apply -f k8s/deployment.yaml")
    ...
    default:
        ...
        sh("kubectl --namespace=${env.BRANCH_NAME} apply -f k8s/deployment.yaml")
